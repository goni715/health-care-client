...
import DoctorScheduleSlots from '@/components/DoctorSchedule/DoctorScheduleSlots';
...
const DoctorsProfilePage = async ({ params }: PropTypes) => {
   const res = await fetch(`http://localhost:5000/api/v1/doctor/get-single-doctor/${params.id}`, {
      next: {
               ...
               <Stack sx={{ bgcolor: 'white', p: 3 }}>
                  <Stack direction='row' gap={3}>
                     <Box sx={{ width: 281, height: 281, bgcolor: '#808080' }}>
                        <Image
                           src={doctor?.profilePhoto || doctor_img}
                           alt='doctor image'
                           width={281}
                           height={281}
                           style={{
